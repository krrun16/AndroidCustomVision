...
public class ImageUtils {
  ...
  private static int fileName_prefix = 1;
  ...
  public static void saveBitmap(final Bitmap bitmap) {
    ...
    i++;
    Log.d("RUMI","Yahooooo saving saving");
    saveBitmap(bitmap, "preview"+i+".png");
  }
  ...
  public static void initializeDirectory() {
    String root =
            Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + "RunningTrack_Test";
    int folder = 1;
    myDir = new File(root+File.separator+folder);
  ...
  public static void saveBitmap(final Bitmap bitmap, List<Classifier.Recognition> results) {
    String label = "";
    double maxScore = -99999;
    if (results != null) {
      for (final Classifier.Recognition recog : results) {
        if (recog.getConfidence() > maxScore) {
          label = recog.getTitle();
          maxScore = recog.getConfidence();
        }
      }
    } else {
      label = "NoMatch";
    ...
    long seconds = (tsLong / 1000) % 60 ;
    String s=Long.toString(seconds);
    long minutes = (tsLong / (1000*60)) % 60;
    String m=Long.toString(minutes);
    ////long hours   = (tsLong / (1000*60*60)) % 24;
    //String h=Long.toString(hours);
    int hour = Calendar.getInstance().get(Calendar.HOUR_OF_DAY);  ///added by me
    String h=Integer.toString(hour);
    //Long tsLong = System.currentTimeMillis()/1000;
    //Long tsLong=System.getTimestamp();

    //String timeStamp = Integer.toString(mSec);
    String timeStamp=h+":"+m+":"+s;


    String filename = fileName_prefix+"_"+timeStamp+"_"+label+".png";

    /*String root =
            Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + "tensorflow";
    LOGGER.i("Saving %dx%d bitmap to %s.", bitmap.getWidth(), bitmap.getHeight(), root);
    File myDir = new File(root+File.separator+folder);
    ...
    */
    ...
    if(myDir != null) {
      saveLog(myDir, timeStamp, label, fileName_prefix, maxScore);
    } else {
      return;
    }
    fileName_prefix++;
    //Log.d("Rumi","saveImage "+timeStamp);
    final String fname = filename;
    final File file = new File(myDir, fname);
    if (file.exists()) {
      file.delete();
  ...
  public static void saveLog(final File myDir, String timeStamp, String label, int prefix, double score) {
    //Log.d("Rumi","saveLog..."+timeStamp);
    ...
    if (file.exists()) {
      try {
        ...
        streamWriter.append(prefix+". ts= "+timeStamp+" label= "+label+ " score= "+score+"\n");
        streamWriter.close();
        out.close();
      } catch (final Exception e) {
        LOGGER.e(e, "Exception!");
